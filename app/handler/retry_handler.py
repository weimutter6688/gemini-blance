            ...
            for attempt in range(self.max_retries):
                try:
                    ...
                    if key_manager:
                        ...
                        new_key = await key_manager.handle_api_failure(old_key, attempt + 1)
                        if not new_key:
                            logger.error(f"No valid API key available after attempt {attempt + 1}")
                            break  # 如果没有有效的API密钥，提前退出重试循环
